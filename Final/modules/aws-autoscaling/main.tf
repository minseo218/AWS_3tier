...
resource "aws_autoscaling_group" "Web-ASG" {
  name = var.asg-name1
  vpc_zone_identifier  = [data.aws_subnet.private1-subnet1.id, data.aws_subnet.private1-subnet2.id]
  launch_template {
...
# Creating Launch template for Was tier AutoScaling Group
resource "aws_launch_template" "Was-LC" {
  ...
  user_data = filebase64("./modules/aws-autoscaling/was.sh")
}
